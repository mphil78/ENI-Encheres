...
public class TraitementProfile extends HttpServlet {
	...
	/**
	 * Redirige vers : -/MonProfile en cas de modificationdu profile avec l'objet
	 * utilisateur en attribut de requete </br>
	 * -/TraitementConnexion en casde suppression apres avoir supprimer
	 * l'utilisateur de la BDD</br>
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (supprimer) {
			...
			if (utilisateurManager.estActif(utilisateur)) {
				request.setAttribute("erreurSuppression",
						"Vous ne pouvez pas supprimer votre compte tant que vous avez des encheres en cours ou des ventes non retirées.");
				request.setAttribute("utilisateurAAfficher", utilisateur);
				RequestDispatcher rd = request.getRequestDispatcher("/MonProfile");
				rd.forward(request, response);
			} else {
				utilisateurManager.delete(utilisateur);
				response.sendRedirect("./TraitementConnexion");
			}
		}
	...
	/**
	 * En cas de creation de compte : Controle l'unicite de l'email et du pseudo
	 * avant d'ajouter l'utilisateur dans la BDD ou de retourner une erreur le cas
	 * échéant</br>
	 * En cas de modification du profile : controle l'unicite de l'email et la
	 * concordance des passwords avant de modifier la BDD</br>
	 * 
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("creation") != null) {
			...
			// si il y au moins une ereeur on renvoie les données de l'utilisateur et les
			// erreurs à la jsp
			if (erreur) {
			...
			// sinon on complète le pojo avant de l'incure dans la bdd, on attribue le
			// pseudo à la session et on renvoie à l'accueil
			else {
		...
		if (request.getParameter("update") != null) {
			...
			boolean erreur = false;
			// détermine si il y a une erreur
			// détermine si l'email choisi existe déjà et que le mot de passe est correct et
			// le cas échant le nouveau mot de passe choisi est ok
			for (Entry<String[], String> user : listePseudosEmails.entrySet()) {
				String pseudoBdd = user.getKey()[0];
				String emailBdd = user.getKey()[1];
				if (!pseudoBdd.equals(session.getAttribute("pseudo"))) {
					...
					if (!UtilisateurManager.hash(request.getParameter("password3"))
							.equals(utilisateurModifie.getMotDePasse())) {
						erreur = true;
						...
						if (!request.getParameter("password4").equals(request.getParameter("password5"))) {
							request.setAttribute("erreurMotDePasse2",
									"Les deux mots de passe ne correspondent pas. Veuillez rectifier.");
							erreur = true;
						}
					}
				}
			}
			// en cas d'erreur on renvoie à la page de modification de profile avec le
			// profile inchangé
			if (erreur) {
