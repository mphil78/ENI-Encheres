...
public class AfficherImage extends HttpServlet {
	...
	private void searchAndWriteArticleImageOn(HttpServletResponse response, final int idArticle) {
		...
		/*
		 * // Example directory on dos system Path dir = Paths.get("c:\\a\\b\\");
		 * 
		 * /**
		 *
		 * Create a new DirectoryStream for the above path.
		 * 
		 * List all files within this directory that begin with the letters A or B i.e
		 * "[AB)]*"
		 * 
		 */
		try (DirectoryStream<Path> stream = Files.newDirectoryStream(Paths.get(TraitementArticle.CHEMIN_IMAGES_ARTICLE),
				String.valueOf(idArticle) + "*")) {
